...
public:
	...
	static constexpr Word TLB_FLOOR_ADDRESS[3] = { 0x40000000, 0x80000000, MAXWORDVAL };
	...
	void setLoadCoreEnabled(bool setting) {
		loadCoreFile = setting;
	}
	bool isLoadCoreEnabled() const {
		return loadCoreFile;
	}

	void setRamSize(Word size);
	Word getRamSize() const {
		return ramSize;
	}
	...
	Word getTLBSize() const {
		return tlbSize;
	}

	void setTLBFloorAddress(Word addr);
	Word getTLBFloorAddress() const {
		return tlbFloorAddress;
	}
	...
	Word getSymbolTableASID() const {
		return symbolTableASID;
	}
